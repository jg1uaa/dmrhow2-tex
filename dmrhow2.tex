...
仮想マシンの用意はこれで完了です。 %\footnote{デフォルトで uim-mozc がインストールされているため、日本語入力ができるようになっています。入力の切り替えは全角/半角キーもしくは Shift+Space で行いますが、他のキーで切り替えたい場合は \tt{uim-pref-gtk3} を使用して設定すると良いでしょう。}
...
\begin{itemize}
 \renewcommand{\labelitemi}{$\triangleright$}
 \item AX.25 は使用しないので \verb+Enable=0+ とします
\end{itemize}
\begin{verbatim}
TXDelay=300
RXTwist=6
...
\begin{itemize}
 \renewcommand{\labelitemi}{$\triangleright$}
 \item \verb+[D-Star]+ で \verb+Enable=0+ としているため、ここは \verb+Enable=1+ のままで構いません (他のプロトコルに関しても同様です)
\end{itemize}
\begin{verbatim}
...
\begin{itemize}
 ...
 \item \verb+LocalAddress=+ の行は削除するか、コメントアウトしてください
\end{itemize}
\begin{verbatim}
LocalPort=62032
RemoteAddress=127.0.0.1
...
\begin{itemize}
 \renewcommand{\labelitemi}{$\triangleright$}
 \item \verb+RemoteAddress=3103.repeater.net+ と \verb+Password=+(BrandMeister の HotSport Security で設定したパスワード) を指定します (コメントアウトも外してください)。\footnote{BrandMeister Master のホスト名は、DMR\_Hosts.txt (\url{http://www.pistar.uk/downloads/DMR_Hosts.txt}) を参照してください。日本向けの TalkGroup はかつて存在した Japan Master でしか使えないという情報がありましたが、これは全くの誤りで、どこの Master に接続しても同等のサービスを受けることができます。}
\end{itemize}
\begin{verbatim}
Jitter=360
Slot1=1
